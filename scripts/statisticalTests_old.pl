...
#Dunn-Sidak correction (m = 630 statistical pairwise comparisons)
#my $alpha = 0.000081415;

#Dunn-Sidak correction (m = 120 statistical pairwise comparisons)
#my $alpha = 0.000427353;

#Dunn-Sidak correction (m = 45 statistical pairwise comparisons)
#my $alpha = 0.001139202;

#Dunn-Sidak correction (m = 15 statistical pairwise comparisons)
#my $alpha = 0.003413713;

#Dunn-Sidak correction (m = 10 statistical pairwise comparisons)
#my $alpha = 0.005116197;

#Dunn-Sidak correction (m = 6 statistical pairwise comparisons)
#my $alpha = 0.008512445;
...
if (($pvalueNormal1 >= $alpha) && ($pvalueNormal2 >= $alpha)){ #Si ambos cumplen el test de normalidad, se tiene que aplicar levene para analizar las varianzas
	#Levene
	my $pvalueLevene = leveneTest($file1, $file2);
	if ($pvalueLevene >= $alpha){ #Varianzas iguales --> anova
...
if ($pValue < $alpha){ #OJO SUPONEMOS MINIMIZACION
