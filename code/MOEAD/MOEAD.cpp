...
// Constructor
MOEAD::MOEAD() {
	secondPopulation = new vector<Individual*>;
}
...
void MOEAD::initialiseWeightVector() {
  weightVector = vector< vector<double> >(getPopulationSize(), vector<double>(getNumberOfObj(), 0));
  ...
  if (numWeightVectors != getPopulationSize()) {
    cerr << "Error MOEA/D: the number of weight vectors does not match with the number of subproblems specified" << endl;
    inputFile.close();
    exit(1);
  }
...
void MOEAD::initialiseNeighbourhood() {
  if (getNeighbourhoodSize() > getPopulationSize()) {
    cerr << "Error MOEA/D: the neighbourhood size (T) cannot be greather than the number of subproblems (N)" << endl;
    exit(1);
  }
...
Individual* MOEAD::createOffspring(const int &i) {
  ...
  // Mutation
  // Potential improvement: mutate p2 and select the best individual from both p1 and p2
  p1->mutation(pm);
  evaluate(p1);
...
void MOEAD::updateSecondPopulation(Individual* ind) {
  ...
  unsigned int i = 0;
  // Removes from the external population all those individuals dominated by individual ind
  while (i < secondPopulation->size()) {
    if (dominanceTest((*secondPopulation)[i], ind) == SECOND_DOMINATES) {
      delete((*secondPopulation)[i]);
      (*secondPopulation)[i] = (*secondPopulation)[secondPopulation->size() - 1];
  ...
  // Adds individual ind to the external population if no individual in the said population dominates it
  bool insert = true;
  i = 0;
  ...
  if (insert)
    secondPopulation->push_back(ind->internalClone());
}

void MOEAD::updateNeighbouringSolution(Individual *offspring, const int &i) {
  for(int j = 0; j < getNeighbourhoodSize(); j++) {
    double id = neighbourhood[i][j]; // the index of the neighbouring subproblem
    double f1 = computingFitnessValue(offspring, weightVector[id]); // fitness of the offspring
    double f2 = computingFitnessValue((*population)[id], weightVector[id]); // fitness of the neighbour
...
void MOEAD::minFastSort(vector<double> &dist, vector<int> &indx, int numberSubProblems, int neighbourhoodSize) {
...
double MOEAD::getEuclideanDistance(vector<double> weightA, vector<double> weightB) {
  double dist = 0;
...
void MOEAD::printInfo(ostream &os) const {
	os << "Multi-objective Evolutionary Algorithm based on decomposition: MOEA/D" << endl;
